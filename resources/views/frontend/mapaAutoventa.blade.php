		...
		<div class="row dashboard">
			...
			<div class="col-sm-2 col-xs-3 border-right">
				<div class="description-block">
					...
					<span class="description-text">Atenciones generadas</span>
				</div>
			</div>
			<div class="col-sm-2 col-xs-3 border-right">
				<div class="description-block">
					...
					<span class="description-text">Atenciones facturadas</span>
				</div>
			</div>
			<div class="col-sm-2 col-xs-4 border-right">
				<div class="description-block">
					...
					<span class="description-text">Motivo de no atencion</span>
				</div>
			</div>
			<div class="col-sm-2 col-xs-4 border-right">
				<div class="description-block">
			...
			*/			
			...
			function sc_obtenerPosicionCliente() {
				...
				.done(function(jsondata) {
					...
					for(var i=0; i<jsondata.length; i++){  md_agregarDatosCliente(jsondata[i]); } 
			...
			function md_agregarDatosCliente(jsondata){
			...
			function sc_actualizarPedidos(mostrarOverlay){			    
				$.ajax({
					type: 'POST',
					url: "{{ url('/') }}/get/pedidos",
					data: { zona: zonaFiltro, ruta : rutaFiltro, fecha : fechaPreventaFiltro, vendedor : vendedorFiltro, guia: "{!! $guia !!}" }
				}).done(function(jsondata) {
			...
			function has_encuesta(p_numeroPedido, p_clientePedido){
				...
				$.ajax({
					type: "POST",
					data: { pedido : p_numeroPedido, cliente : p_clientePedido },
					dataType: 'json',
	...
	/*
        ...
        function md_actualizarDashboard(){			
			$.ajax({
				...
				type: "POST",
				dataType: 'json',
				data: { "ruta" : rutaFiltro,"zona" : zonaFiltro,"fecha" : fechaPreventaFiltro,"vendedor": {{ $vendedor }} },
				url: "{{ url('/') }}/get/{{ $metodo }}"
			})
			.done(function(jsondata) {
